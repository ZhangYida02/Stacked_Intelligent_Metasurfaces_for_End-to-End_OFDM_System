...
class EMNN_tx(nn.Module):
    ...
    def forward(self, x, args):
        ...
        TxSim_transmit_complex = torch.from_numpy(np.zeros((args['M'] * args['N_c'], m), dtype=np.complex64)).cuda()
        for n_c in range(args['N_c']):
            temp = self.WTxA[:, :, n_c] @ x_plural[n_c * args['A_tx']: (n_c + 1) * args['A_tx'], :]
            for i in range(self.L):
                phase=self.RIS[i](torch.tensor(0).cuda())
                Ris_coefficient = torch.diag(torch.exp(1j*phase)).cfloat()
                temp = Ris_coefficient @ temp
                if i != self.L-1:
                    temp = self.WTxSim[:,:,i,n_c] @ temp
            TxSim_transmit_complex[n_c * args['M']: (n_c + 1) * args['M'], :] = temp
        TxSim_transmit = torch.transpose(TxSim_transmit_complex, 1, 0)
...
class EMNN_rx(nn.Module):
    ...
    def forward(self, x0, args):
        ...
        x_plural = torch.transpose(x_plural, 1, 0).cfloat()
        RxSim_receive_complex = torch.from_numpy(np.zeros((args['A_rx'] * args['N_c'], m), dtype=np.complex64)).cuda()
        for n_c in range(args['N_c']):
            temp = x_plural[n_c * args['N']: (n_c + 1) * args['N'], :]
            for i in range(self.K - 1, -1, -1):
                phase=self.RIS[i](torch.tensor(0).cuda())
                Ris_coefficient = torch.diag(torch.exp(1j*phase)).cfloat()
                temp = Ris_coefficient @ temp
                if i != 0:
                    temp = self.WRxSim[:,:,i-1,n_c] @ temp
            temp = self.WRxA[:, :, n_c] @ temp
            RxSim_receive_complex[n_c * args['A_rx']: (n_c + 1) * args['A_rx'], :] = temp
