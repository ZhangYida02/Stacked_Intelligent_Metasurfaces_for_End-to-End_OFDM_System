...
def get_txSim(args):
    XTxSim = np.linspace(-args['Lam_c'] / 2 * (args['M1'] - 1) / 2, args['Lam_c'] / 2 * (args['M1'] - 1) / 2, args['M1'])
    YTxSim = np.linspace(-args['Lam_c'] / 2 * (args['M2'] - 1) / 2, args['Lam_c'] / 2 * (args['M2'] - 1) / 2, args['M2'])
    ZTxSim = np.linspace(args['dzTx'], args['dzTx'] * args['L'], args['L'])

    DTxSim = np.zeros((args['M'], args['M'], args['L'] - 1))
    ThetaTxSim = np.zeros((args['M'], args['M'], args['L'] - 1))
    WTxSim = np.zeros((args['M'], args['M'], args['L'] - 1, args['N_c']),dtype=np.complex64)
        ...
        for i in range(args['M']):
            for j in range(args['M']):
                ...
                for n_c in range(args['N_c']):
                    WTxSim[j, i, l, n_c] = args['dxTx'] * args['dyTx'] * np.cos(ThetaTxSim[j, i, l]) / DTxSim[j, i, l] * (1 / (2 * np.pi * DTxSim[j, i, l]) - 1j /args['Lam'][n_c]) * np.exp(1j * 2 * np.pi * DTxSim[j, i, l] / args['Lam'][n_c] )


    XTxA = np.linspace(-args['Lam_c'] / 2 * (args['A_tx1'] - 1) / 2, args['Lam_c'] / 2 * (args['A_tx1'] - 1) / 2, args['A_tx1'])
    YTxA = np.linspace(-args['Lam_c'] / 2 * (args['A_tx2'] - 1) / 2, args['Lam_c'] / 2 * (args['A_tx2'] - 1) / 2, args['A_tx2'])
    DTxA = np.zeros((args['M'], args['A_tx']))
    ThetaTxA = np.zeros((args['M'], args['A_tx']))
    WTxA = np.zeros((args['M'], args['A_tx'], args['N_c']),dtype=np.complex64)
...
def get_rxSim(args):
    XRxSim = np.linspace(-args['Lam_c'] / 2 * (args['N1'] - 1) / 2, args['Lam_c'] / 2 * (args['N1'] - 1) / 2, args['N1'])
    YRxSim = np.linspace(-args['Lam_c'] / 2 * (args['N2'] - 1) / 2, args['Lam_c'] / 2 * (args['N2'] - 1) / 2, args['N2'])
    ZRxSim = np.linspace(args['dzRx'], args['dzRx'] * args['K'], args['K'])

    DRxSim = np.zeros((args['N'], args['N'], args['K'] - 1))
    ThetaRxSim = np.zeros((args['N'], args['N'], args['K'] - 1))
    WRxSim = np.zeros((args['N'], args['N'], args['K'] - 1, args['N_c']),dtype=np.complex64)
    for k in range(args['K']-1):
        for i in range(args['N']):
            for j in range(args['N']):
                ...
                for n_c in range(args['N_c']):
                    WRxSim[j, i, k, n_c] = args['dxRx'] * args['dyRx'] * np.cos(ThetaRxSim[j, i, k]) / DRxSim[j, i, k] * (1 / (2 * np.pi * DRxSim[j, i, k]) - 1j /args['Lam'][n_c]) * np.exp(1j * 2 * np.pi * DRxSim[j, i, k] / args['Lam'][n_c] )


    XRxA = np.linspace(-args['Lam_c'] / 2 * (args['A_rx1'] - 1) / 2, args['Lam_c'] / 2 * (args['A_rx1'] - 1) / 2, args['A_rx1'])
    YRxA = np.linspace(-args['Lam_c'] / 2 * (args['A_rx2'] - 1) / 2, args['Lam_c'] / 2 * (args['A_rx2'] - 1) / 2, args['A_rx2'])
    DRxA = np.zeros((args['A_rx'], args['N']))
    ThetaRxA = np.zeros((args['A_rx'], args['N']))
    WRxA = np.zeros((args['A_rx'], args['N'], args['N_c']), dtype=np.complex64)
