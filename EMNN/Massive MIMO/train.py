...
def train(args, G, NN_tx, NN_RX):
    ...
    for epoch in range(args['train_eps']):
        ...
        RxSim_receive = Channel(x_transmit, G, args['J'], args)

        r_error = 0
        for j in range(args['J']):
            RxSim_receive0 = torch.cat([RxSim_receive[j].real, RxSim_receive[j].imag], dim=1)
            y_receive = RxSim_receive0 + Noise_train[j]
        ...
        if epoch % 100 == 0:
            ...
            for i_P_tx in range(args['P_tx_vali_dbm'].shape[0]):
                ...
                y_pred = test(X_vali, args, args['P_tx_vali_dbm'][i_P_tx], NN_tx, NN_RX, G)
