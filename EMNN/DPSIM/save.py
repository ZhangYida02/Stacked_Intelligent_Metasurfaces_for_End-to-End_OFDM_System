...
def save_all_models(NN_tx, DPSIM_tx, DPSIM_Rx, NN_RX, pathname_prefix="all_models"):
    ...
    save_dict = {
        ...
        'DPSIM_tx': DPSIM_tx.state_dict(),
        'DPSIM_Rx': [m.state_dict() for m in DPSIM_Rx],
...
def load_all_models(NN_tx, DPSIM_tx, DPSIM_Rx, NN_RX, pathname_prefix="all_models", device="cpu"):
    ...
    DPSIM_tx.load_state_dict(_remove_module_prefix(checkpoint['DPSIM_tx']))
    for i, m in enumerate(DPSIM_Rx):
        m.load_state_dict(_remove_module_prefix(checkpoint['DPSIM_Rx'][i]))
