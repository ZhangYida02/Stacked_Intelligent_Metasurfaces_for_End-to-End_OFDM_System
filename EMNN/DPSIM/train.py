...
def train(args, G, NN_tx, DPSIM_tx, DPSIM_RX, NN_RX):


    NN_tx = torch.nn.DataParallel(NN_tx).cuda()
    DPSIM_tx = torch.nn.DataParallel(DPSIM_tx).cuda()
    for j in range(args['J']):
        DPSIM_RX[j] = torch.nn.DataParallel(DPSIM_RX[j]).cuda()
    ...
    DPSIM_tx_optimizer = torch.optim.AdamW(DPSIM_tx.parameters(), lr=t_learning_rate, betas=optim_betas)
    DPSIM_RX_optimizer = []
    NN_RX_optimizer = []
    for i in range(args['J']):
        DPSIM_RX_optimizer.append(torch.optim.AdamW(DPSIM_RX[i].parameters(), lr=t_learning_rate, betas=optim_betas))
    ...
    for epoch in range(args['train_eps']):
        ...
        Noise_train = []
        for j in range(args['J']):
            Noise_train.append(torch.from_numpy(np.random.randn(args['batch_size'], 2 * args['A_rx_dp'] * args['N_c'] * 2) * np.sqrt(args['Noise']) / np.sqrt(2)).float().cuda())

        NN_tx.zero_grad()
        DPSIM_tx.zero_grad()
        for j in range(args['J']):
            DPSIM_RX[j].zero_grad()
            NN_RX[j].zero_grad()
        ...
        TxSim_transmit = DPSIM_tx(x_transmit, args)
        ...
        for j in range(args['J']):
            ...
            y_receive = DPSIM_RX[j](RxSim_receive0, args) + Noise_train[j]
        ...
        for j in range(args['J']):
            ...
            DPSIM_RX_optimizer[j].step()
        DPSIM_tx_optimizer.step()
        ...
        if epoch % 100 == 0:
            ...
            DPSIM_tx_optimizer.param_groups[0]['lr'] /= args['LR_Factor']
            for i in range(args['J']):
                DPSIM_RX_optimizer[j].param_groups[0]['lr'] /= args['LR_Factor']
            ...
            for i_P_tx in range(args['P_tx_vali_dbm'].shape[0]):
                ...
                y_pred = test(X_vali, args, args['P_tx_vali_dbm'][i_P_tx], NN_tx, DPSIM_tx, DPSIM_RX, NN_RX, G)
    ...
    return NN_tx, DPSIM_tx, DPSIM_RX, NN_RX,R_error