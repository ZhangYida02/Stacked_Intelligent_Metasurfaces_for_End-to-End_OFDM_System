...
print('---------------------------------------------SIM---------------------------------------------')
txSim=system.get_txDpSim(args)
rxSim=system.get_rxDpSim(args)

print('---------------------------------------------channel---------------------------------------------')
G = channel.get_channel(args)

print('---------------------------------------------model---------------------------------------------')
NN_tx, DPSIM_tx, DPSIM_Rx, NN_RX = model.get_model(args, txSim, rxSim)
...
print('---------------------------------------------train---------------------------------------------')
NN_tx, SIM_tx, DPSIM_Rx, DPNN_RX,R_error= train(args, G ,NN_tx, DPSIM_tx, DPSIM_Rx, NN_RX)
...
for p_tx_test in range(args['P_tx_test_dBm'].shape[0]):
    ...
    for test_index in range(args['Num_test']//args['batch_size_test']):
        Y_pred[test_index * args['batch_size_test']: (test_index + 1) * args['batch_size_test'], :] = test(X_test[test_index * args['batch_size_test']: (test_index + 1) * args['batch_size_test'], :], args, args['P_tx_test_dBm'][p_tx_test], NN_tx, DPSIM_tx, DPSIM_Rx, NN_RX, G)
...
print('---------------------------------------------save---------------------------------------------')
if args['channel'] == 'statistics.mat':
    save_all_models(NN_tx, DPSIM_tx, DPSIM_Rx, NN_RX, args['pathname_prefix'])
else:
    print(f"不保存模型")
